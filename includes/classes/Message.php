<?php
	class Message{
		...
		public function getLatestMessage($userLoggedIn, $user2){
			...
			$query = mysqli_query($this->con, "SELECT body, user_to, user_from, date FROM messages WHERE (user_to='$userLoggedIn' AND user_from='$user2') OR (user_to='$user2' AND user_from='$userLoggedIn') ORDER BY id DESC LIMIT 1");

			$row = mysqli_fetch_array($query);
			$user_from = $row['user_from'];
			$sent_by = ($row['user_to'] == $userLoggedIn) ? "$user_from said: " : "You said: ";
