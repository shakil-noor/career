...
<body>
	...
	<?php 
		...
		if(isset($_SESSION['username'])){
			...
			//var_dump ($user_details_query); exit;
	...
	<?php 
		...
		if($count !=0){
			while($comment = mysqli_fetch_array($get_comments)){
				...
				if($interval->y >=1){ //year count
					if($interval == 1)
						$time_message = $interval->y. " year ago"; //1 year ago 
					else
						$time_message = $interval->y . " years ago"; //1+ years ago 
				}
				else if($interval-> m >=1){ //month count
					if($interval->d == 0){
						$days = " ago";
				...
				else if($interval->d >=1){ //day count
					if($interval->d == 1){
				...
				else if($interval->h >=1){ // hours count 
					if($interval->h == 1){
				...
				else if($interval->i >=1){ // minutes count
					if($interval->i == 1){
